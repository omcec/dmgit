...
* Neste módulo é onde faz o registro prévio das vendas para depois ser efetivado no **Terminal Caixa**. Mas também é possível fazer a venda direto no Caixa, direto no **DMaster ECF**, o que veremos mais para frente.
* Você vai ter um Terminal balcão e um Terminal caixa?
...
* No caso de quem vai trabalhar apenas com uma máquina, pode fazer a venda direto no ECF.
  * ​​​​​​​Salvo algumas exceções que devem ser realizadas no pré-vendas:
    ...
    * Venda de entrega.
* Então você criando a pré-venda aqui ela já fica disponível para efetivar no caixa.
* Ao **adicionar um produto** no grid, o **estoque já diminui** para não dar conflito, caso outro atendente esteja vendendo o mesmo produto de estoque baixo.
* **Primeiro: vamos inserir um produto para fazer o processo mais simples de venda**:
* Pode inserir informando o código interno do produto, código de barras (usando o leitor ou digitando) ou digitando o nome do produto.
* **F2**: seleciona a forma de pagamento em dinheiro. Com isso você deve selecionar o atendente e depois só dando OK até confirmar a pré-venda.
* Cada atendente deve saber seu código de cadastro, pois esse campo não dá para pesquisar pelo nome.
* Inserir outro produto.
* Fazer uma venda para mostrar a aplicação dos descontos direto na linha do produto no grid ou na venda inteira com o **F11** na tela de pagamentos.
* Verificar se eles pretendem gerenciar a entrega pelo sistema. Caso não, pular o próximo passo.
...
* Já foi cadastrado algum cliente vinculado a algum convênio para vendas no crediário a prazo?
...
* **F4 - Crédito, débito, PIX**: nessas modalidades de pagamentos você vai selecionar o cartão ou o PIX. O campo **Nº do Documento** referente ao número do comprovante da maquininha do cartão, não é obrigatório. E no campo **Parcelas**, quantidade da parcela.
* **F8 - Convênios On Line – PBM's**: são para as vendas de PBMs como farmácia popular e outro programas de benefícios (Funcional Card, e-Pharma, Portal da Drogaria).
* **F9 - Clientes**: serve simplesmente para consultar ou selecionar o cliente antes de iniciar a venda.
...
* **F10** - acesso a itens: ao teclar F10 ou clicar no item mostra a porcentagem do desconto aplicado e o valor mínimo do item.
* **F11**: aplicação de desconto após selecionar a forma de pagamento. Caso a venda seja de entrega, primeiro você informa o desconto e depois informa a entrega no F6.
* **F12**: **sem nenhum produto lançado**, serve para ver todas as pré-vendas lançadas, os produtos da pré-venda e a possibilidade de excluir a pré-venda ou um item da pré-venda.
* **F12**: **na tela de fechamento** da pré-venda, serve para **emitir NFe** em vez de Cupom fiscal. Caso precise, pode abrir um chamado, pois o processo é finalizado em outra aplicação.
* **Shift+F12** - **Manutenção de pedidos**: para montar um pedido com os produtos que estão em falta ou até mesmo para encomendar um produto. Os pedidos por falta ou por encomenda podem ser criados aqui e vistos em Relatório > Manutenção de Pedidos.
* É possível também incluir um produto em uma pré-venda existente.
...
* **F1**: aqui você vê o resumo dos comandos desse módulo.
...
1. \
   Esse módulo serve para devolver a quantidade do produto ao estoque.
2. A devolução só pode ser feita de forma integral. Caso a venda tenha mais itens e o cliente não esteja devolvendo tudo, será necessário refazer a venda.
   * _Somente nos casos em que houver um questionamento do cliente e se ele for ter SNGPC integrado:_
     * _É possível fazer a devolução parcial, porém o cancelamento do cupom ainda será integral._
...
1. \
   Este módulo é responsável pelo **auxílio financeiro**, ou seja, quanto aos **pagamentos de contas da sua loja**. Aqui é possível baixar, excluir e incluir novas contas, além de alterar valor e vencimento de conta.
...
* Este módulo serve para registrar a entrada ou saída do estoque de forma manual de acordo com o evento.
* Basta selecionar o Evento.
* Inserir o produto no campo código.
* Informar a quantidade de Unidades ou de Frações.
* Outra função é, para aqueles que possui o modo **Vender Quantidade Maior Que o Estoque Disponível**​​​​​​​ ativo, poderá fazer o ajuste de zerar todos os produtos que estiverem com estoque negativo, inclusive também poderá desativar.
...
1. \
   Este módulo serve para efetuar as **transferências de produtos** entre lojas vinculadas. As lojas vinculadas não precisam ser necessariamente matriz e filial.
...
3. **Transferência por Nota Fiscal**: é gerado um NFe nesse tipo de transferência. Após informar os itens da transferência, a nota deverá ser emitida no NFe. Para receber, o mesmo deve ser feito no módulo de Entrada de NFe.
   * Somente as transferências por NFe tem valor fiscal.
   * Só é permitido realizar a transferência por nota fiscal entre lojas que são filiais no CNPJ.
...
* Este módulo serve para dar baixa nos pagamentos feito pelos cartões e PIX.
...
* Para dar baixa, basta selecionar as vendas, marcando com o sinal de **+** do teclado e depois clicar em **Gravar**.
* É possível pesquisar por vencimento ou vendas já baixadas e pelo tipo de pagamento.
