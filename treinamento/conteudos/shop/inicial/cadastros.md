...
3. Questionar se o cliente **irá gerenciar as entregas pelo sistema**, em caso positivo, explicar o parâmetro **Entregador**: já existe um cadastro para representar todos os entregadores. Ou você cadastra cada entregador para uma gestão melhor das entregas.
4. ​​​​​​​​​​​​​​Explicar o parâmetro **Inativo** e botão **Marcar Todos**.
5. Aconselhar a **inativar o usuário Atendente**, ou pelo menos **trocar a senha** por estar com uma senha padrão para todos os clientes.
6. Informar que cadastrar um **usuário sem marcar nenhuma opção** na aba **Controle de Acessos** é ideal para um **atendente** que vai ficar **apenas no caixa** e para o perfil de **entregador**.
7. Logo em seguida sair do sistema e logar com o usuário dele.
8. Questionar se quer cadastrar outro usuário.
9. Informar ao cliente sobre os parâmetros de **Controle de Acesos** e fazendo analogia de que a coluna da Seção equivale aos módulos do sistema para o cliente ter noção.
...
A classe serve para definir a que grupo pertence um determinado tipo de produto. Inclusive, quando você cadastra um produto manualmente pelo módulo de **Cadastro** de **Produto**, é obrigatório escolher a qual classe ele deverá pertencer.

1. Quando você for **cadastrar uma classe**, você já deve colocar o nome que você quer no campo **Descrição,** marcar o parâmetro **Classe Ativa** e **Gravar**. É o mínimo que o sistema pede.
2. Aqui também é possível cadastrar uma **subclasse para subdividir** e fazer um micro gerenciamento.
   * <mark style="color:blue;">**Por exemplo: tente dar um exemplo conforme o tipo de comercio do cliente.**</mark>
   * Para a classe Biscoito você pode criar uma subclasse de Doces e Salgados. Mas não necessariamente essa subclasse não estará vinculada a classe, pois ela poderá ser usada por outras classes também.
     * Isso é bom para organizar, por exemplo, diferentes comissões e descontos só para aqueles produtos daquela subclasse.
     * E essa separação de subclasse você define em cadastro dos produtos. Quando a gente for ver como cadastrar um produto, vou te mostrar.
3. A configuração do desconto não é fixo, é o desconto máximo permitido. O percentual máximo de desconto é responsável por limitar o usuário de conceder um desconto na venda além do estipulado pelo gestor do estabelecimento, caso ultrapasse o limite será solicitado o usuário e senha de quem contenha a permissão **"Liberações - Desconto superior ao permitido".**\
...
4. Já o campo de **Comissão x Desconto** serve para, justamente, definir a porcentagem de comissão conforme a porcentagem de desconto.
   * Ela é inversamente proporcional.
     ...
     * Ou para ele receber o máximo de comissão, ele não poderá atribuir nenhum desconto.
     * Mas isso é ajustável, você que define o mínimo e o máximo de desconto e de comissão.
5. Esses parâmetros de “**Ajustar preço de custo**” e “**Ajustar preço de venda**”: não são utilizados.​​​​​​​
6. <mark style="color:blue;">**O passo abaixo deve ser passado conforme percepção da necessidade do cliente.**</mark>
...
<summary><mark style="color:red;">*</mark> FORNECEDORES</summary>
...
2. Então nesse campo **Código é o campo padrão de pesquisa de produto**. Basta colocar **pelo menos três letras** para pesquisar ou o **\*** para carregar todos os itens
...
5. No campo **código de barras**, ao dar entrada na nota fiscal, ele será preenchido automaticamente, mas é possível alterar e incluir um outro código de barras, caso o produto tenha mais de um código de barras.
6. **NCM**: Nomenclatura Comum do Mercosul, trata-se de um código estabelecido pelo Governo Brasileiro para identificar a natureza das mercadorias e assim compor o imposto.
7. **CEST**: Código Especificador da Substituição Tributária, estabelece uma forma de identificar e uniformizar mercadorias e bens sujeitos ao regime de substituição tributária que também compõe o imposto.
8. **CFOP**: é preenchido automaticamente ao escolher o ICMS, mas é possível alterá-los caso você receba alguma orientação da contabilidade.
9. **Descrição**: é nome que aparece no momento da venda. Para produtos fracionados, pode ser no máximo 24 caracteres.
10. **Informações adicionais do produto**: as informações contidas aqui, poderão ser visualizadas no caso de emissão de NFe.
11. **Custo atual**: valor de compra conforme a entrada da NFe, porém é possível editar.
12. **Custo médio**: valor médio de custo das compras dos últimos 90 dias.
13. **%Markup**: informa o ganho em cima do custo. Exemplo: qual a porcentagem você quer ganhar em cima do custo? Se um produto tiver custo de R$ 10,00 e você quiser ter 100% de ganho em cima do custo, o percentual do Markup deverá ser 100%, com isso, o preço final será o dobro do valor do custo, ou seja R$ 20,00.
14. **Lucro**: informa o ganho em cima do custo atual em relação ao preço de venda. Aproveitando o exemplo acima, o lucro neste caso será de 50%, ou seja, a metade do valor da venda refere-se ao custo.
15. **Preço**: preço de venda do produto.
16. **Preço c/Desc**: aqui você pode definir o preço com desconto fixo (ou preço promocional), ou temos um outro módulo que serve para definir o desconto de vários produtos ao mesmo tempo ou todos os produtos de uma classe. Ao realizar o processo pelo outro módulo, o valor com desconto aparecerá nesse campo.
17. **Desc%**: desconto máximo permitido. Prevalece ao desconto da Classe.
18. **Comissão%**: de venda para o atendente. Prevalece ao desconto da Classe.
19. **Fabricantes**: serve para informar o Fabricante do produto.
    * ​​​​​​​Para colocar um novo, deve cadastrá-lo no **atalho F3**.
20. ​​​​​​​**Classes**: serve para informar a Classe, para colocar um novo, deve cadastrá-lo no **atalho** **F4.**
21. **Subclasses**: não é obrigatório informar a subclasse. Para cadastrar também é no **atalho** **F4.**
22. **Fracionado**: para poder fracionar o produto.
23. **ICMS**: refere-se a alíquota do ICMS do produto. Neste caso, havendo dúvida, deve ser verificado com Contabilidade a alíquota correta.
24. **CSOSN**: é preenchido automaticamente ao escolher o ICMS, mas é possível alterá-los caso você receba alguma orientação da contabilidade.
25. **Lista PIS/COFINS**: caso não tenha familiaridade também deve ser verificado com a contabilidade. Contudo, existe um padrão:
    * **Lista Positiva - Isento**: são os medicamentos em geral.
    * **Lista Negativa - Monofásico**: todos os produtos que possuem mistura química, mistura homogênea ou volume do tipo (ML ou Gramas). Exemplos: shampoos, condicionadores, esmaltes, alisantes, tinturas etc. Obs.: Praticamente todos os itens de perfumaria.
...
26. **Imp. União e Imp. Estado**: imposto recolhido. Preenchido conforme no NCM, mas também pode ser alterado. Eles aparecem no cupom fiscal.
    * Caso o campo não esteja atualizando conforme o NCM, deve ser atualizado a tabela no módulo **Atualizar** > **Tabela IBPTax**.
27. **Gr. Sugestão**: o sistema permite sugerir um produto no momento da venda de outro produto. O sistema possui um módulo para criar um grupo que vamos ver mais pra frente. Nisso você colocar o número do grupo aqui em pelo menos dois produtos para que, ao lançar um deles, o sistema indique o outro na hora da venda para você oferecer para o cliente.
28. CAMPOS REFERENTE AO FRACIONAMENTO
    * ​​​​​​​Antes do fracionamento, devemos zerar o estoque para fazer a contagem correta e não apresentar erro.
    * Alterar o parâmetro **Fracionado** para **SIM**.
...
29. **Peso**: não está em uso.
30. **Localização**: serve para colocar a localização do produto na loja. Em qual armário, gaveta, seção, corredor ou estante... etc.
31. **Usuário Última Alt**.: último usuário que alterou o cadastro.
32. **Produto ativo**: com esse parâmetro desmarcado, ele sai da lista de pesquisa padrão.
...
33. **Produção**: é usado para produtos que sejam de produção da própria loja. Ao efetuar a venda, será emitido um cupom extra em outra impressora no local de produção (lanchonete - cozinha).
34. **Serviço Agregado**: esse parâmetro serve para os produtos que são vendidos por medida (metragem). Com isso na hora da venda, é solicitado a quantidade a ser vendida.
35. **Venda rápida**: não está em uso.
36. **Exportar balança**: usado para produtos que precisam ser pesados em uma balança externa ou acoplada no caixa. Mais a frente vou mostrar como gerar os arquivos desses produtos para você poder exportar para a balança.
37. **Dias de validade**: utilizado somente para balança Toledo. A quantidade de dias informado será impressa na etiqueta da balança.
38. **Ult. Alteração**: data da última alteração efetuada no cadastro.
39. **Ult. Compra**: data conforme última entrada de nota.
40. **Ult. Venda**: data conforme a última venda efetivad&#x61;**.**
41. **Demanda**: aqui aparece quantidade vendida nos últimos 90 dias.
42. **UN**: mostra a quantidade do estoque. Em caso de produto fracionado, mostra a quantidade de caixas ou embalagens.
43. **Frações**: mostra as quantidades de frações. ​​​​​​​
44. **F12 - Dados do último produto:** serve para quando for cadastrar um produto similar ao último cadastrado, para que não tenha que preencher todos os dados novamente.
45. ​​​​​​​**GRAVAR**
...
<summary>PREÇO LOJA - Integração</summary>
...
4. No momento da venda, quando você colocar o produto ele vai te dar a opção de vender a unidade ou o kit.
...
2. No campo **Código do kit**: pesquisar pelo nome ou pelo código do kit criado.
...
<summary>ÍNDICE TÉCNICO DE PRODUÇÃO</summary>

1. Esse módulo serve para cadastrar os itens que utilizam produtos “inclusos” em outro. Exemplo: açaí (amendoim, granulado, granola, cobertura).
2. **Cód. Principal**: é o produto principal que será vendido. Ele deve estar cadastrado como produto com o parâmetro Produção no cadastro (não precisa ter estoque). Apenas os produtos incluídos nos insumos que irão diminuir o estoque.
3. **Cód. Insumo**: são os produtos que serão incluídos para montar o produto principal.
4. Quantidade.
...
1. É utilizado para fazer vendas casadas. A função deste módulo é sugerir um produto ao vender um outro produto. Então quando você for lançar um determinado produto na pré-venda, o sistema irá sugerir um outro produto conforme você definir os produtos dentro do mesmo grupo de sugestão.
2. Primeiro você deve **criar um grupo** dando um nome e clicar em gravar.
3. É possível pesquisar o grupo cadastrado através do campo código.
4. Ir em **Cadastro de Produto**, pesquisar e selecionar o produto que você queira inserir no grupo de sugestão.
5. No campo **Gr. Sugestão**: pesquisar o grupo que você criou e selecionar ele.
...
3. **Convênio empresa**: neste você cadastra o convênio de parceria com outra empresa. Onde os funcionários dessa outra empresa poderão comprar com você e depois a empresa conveniada irá efetuar o pagamento para você em uma data específica. Pagamento é à prazo, e pode ser parcelado ou não.
4. **Convênio Entregas**: é bom ter esse cadastro se você for trabalhar com entregas, pois para registrar uma entrega pelo sistema, é necessário ter o cadastro do cliente, e para cadastrar um cliente é obrigatório vincular ele a algum convênio.
...
1. **Desc. Geral**: é aplicado em todos os produtos no momento da venda para o cliente deste convênio.
...
9. **Imprimir cupom**: vinculado dos cartões de crédito (mostra as parcelas), débito e PIX.
...
1. Para cadastro de clientes, é **necessário** pelo menos o **nome** e **convênio**.
   * E para clientes de Convênio Entrega, é obrigatório informar um número de telefone.
...
7. **Segundo endereço**: campo para informar um segundo endereço, caso haja.
8. **Convênio**: é obrigatório selecionar um convênio, seja à prazo ou à vista.
9. **Limite de crédito**: informa o valor máximo de compra do cliente de convênio à prazo.
10. **Crédito atual**: atualiza a limite de compra conforme o uso e os pagamentos.
...
2. Setores é utilizado para que, ao cadastrar um novo cliente, poder informar qual a loja mais próxima do mesmo, para melhor atendê-lo no caso de vendas de entrega.
3. O sistema já possuí alguns setores cadastrados (Centro, Norte, Sul, Leste, Oeste e Centro Oeste), **podendo já serem utilizados, bastando apenas selecionar o setor e indicar a filial a ser associada**.
...
1. Serve para cadastrar as despesas da loja para ter um maior controle dos gastos. Como por exemplo: despesa com pessoal, fornecedores de energia, água, internet.
